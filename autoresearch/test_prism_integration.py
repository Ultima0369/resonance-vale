...
def test_ethics_report(engine):
    ...
    print(f"批准决策: {report['approved_decisions']}")
    print(f"拒绝决策: {report['rejected_decisions']}")
    print(f"批准率: {report['approval_rate']:.1%}")
    
    avg = report['average_scores']
    print(f"平均评分: 红={avg['red']:.3f}, 蓝={avg['blue']:.3f}, 紫={avg['purple']:.3f}, 总体={avg['overall']:.3f}")
...
def test_state_persistence():
    ...
    try:
        ...
        if len(history1) == len(history2):
            ...
            print(f"❌ 状态加载失败: 历史长度不一致 ({len(history1)} vs {len(history2)})")
...
def test_autoresearch_integration():
    ...
    for i, plan in enumerate(experiment_plans, 1):
        ...
        status = "✅ 批准" if should_proceed else "❌ 拒绝"
        print(f"  计划{i}: {plan['objective'][:30]}... - {status} (评分: {score.overall:.3f})")
    
    print(f"\n📈 审批统计: {sum(1 for r in results if r['approved'])}/{len(results)} 个计划被批准")
...
def test_fire_side_dialogues():
    ...
    dialogues = engine.get_fire_side_dialogues()
    print(f"\n添加后对话数: {len(dialogues)}")
    
    return len(dialogues)

def main():
    """主测试函数"""
    print("🚀 开始棱镜协议与autoresearch集成测试")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    if passed == total:
        print("🎉 所有测试通过！棱镜协议成功集成到autoresearch")
        print("🔥 现在可以开始伦理增强的自主研究了")
    else:
