...
try {
    ...
    if ($feed.posts.Count -gt 0) {
        Write-Host "Latest posts ($($feed.posts.Count) items):" -ForegroundColor Green
        ...
        Write-Host "No new posts" -ForegroundColor Gray
    }
} catch {
...
Write-Host "Next check recommended: 2-3 hours later" -ForegroundColor Cyan